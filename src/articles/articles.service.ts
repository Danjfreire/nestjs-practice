...
export class ArticlesService {
  ...
  async listArticles(
    ...
    try {
      const [author, requester] = await Promise.all([
        queryOptions.query.author
          ? this.userService.findByUsername(queryOptions.query.author)
          : null,
  ...
  async getArticle(slug: string, requesterId?: string): Promise<ArticleJSON> {
    const user = requesterId
      ? await this.userService.findById(requesterId)
      : null;
  ...
  async favoriteArticle(
    ...
    const [updatedUser, updatedArticle] = await Promise.all([
      this.userService.addFavoriteArticle(
        requesterId,
        (article as ArticleDocument)._id,
      ),
  ...
  async unfavoriteArticle(
    ...
    const [updatedUser, updatedArticle] = await Promise.all([
      this.userService.removeFavoriteArticle(
        requesterId,
        (article as ArticleDocument)._id,
      ),
      (await article.save()).populate('author'),
